...
powershell -executionpolicy ByPass ^
    ...
    $log_line = (Get-Date).ToString('yyyy-MM-dd hh:mm:ss')+' active-response/bin/remove-file-vt.cmd: Starting with Alert ID '+$alert_id; ^
    ...
    if ($alert_dict.parameters.alert.data.integration -eq 'virustotal') { ^
        ...
        $log_line = (Get-Date).ToString('yyyy-MM-dd hh:mm:ss')+' active-response/bin/remove-file-vt.cmd: '; ^
        if ($result.Count -eq 0) {$log_line += $filename+' deleted'} else {$log_line += 'Unable to delete. Reason: '+$result[0].CategoryInfo.Reason}; ^
        $log_line ^| Out-File -FilePath $log_file -Append -Encoding ASCII} ^
    else { ^
        $log_line = (Get-Date).ToString('yyyy-MM-dd hh:mm:ss')+' active-response/bin/remove-file-vt.cmd: Not a VirusTotal Alert: '+$alert}; ^
    $log_line = (Get-Date).ToString('yyyy-MM-dd hh:mm:ss')+' active-response/bin/remove-file-vt.cmd: Ended'; ^
    $log_line ^| Out-File -FilePath $log_file -Append -Encoding ASCII
